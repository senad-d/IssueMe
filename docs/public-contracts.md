...
| `issueme_add_issue_to_project` | No | REST issue check + GraphQL Projects v2 project identity preflight/add mutation | None | sequential | Throws validation/project-owner mismatch/creator-scope/closed-issue/API failures before add mutation; already-present item is success | projects, integration |
| `issueme_update_project_item` | No | REST issue check + GraphQL Projects v2 | None | sequential | Throws validation/creator-scope/closed-issue/target-mismatch/API failures | projects, integration |
...
| `issueme_create_issue` | No | REST issues + authenticated-user lookup when restricted | Writes created issue cache file | sequential | Throws before remote create, including creator-scope token-user mismatch; cache write failure after create returns `partial_success` | sanitization, integration, partial success, failure semantics |
...
| `issueme_add_sub_issue` | No | REST checks + GraphQL native sub-issues | Refreshes relationship cache | sequential | Creator-scope or closed-issue refusal throws; handled native attach failure returns `result:error`; cache refresh failure returns `partial_success` | sub-issue, integration |
| `issueme_remove_sub_issue` | No | REST checks + GraphQL native sub-issues | Refreshes relationship cache | sequential | Creator-scope or closed-issue refusal throws; handled native remove failure returns `result:error`; cache refresh failure returns `partial_success` | sub-issue, integration |
| `issueme_reorder_sub_issues` | No | REST/GraphQL native sub-issues | Refreshes relationship cache | sequential | Creator-scope failures throw; native reorder no-op is success; handled native failures return `result:error`; cache refresh failure returns `partial_success` | sub-issue, integration |
...
| `issueme_reopen_issue` | No | REST issue reopen/comment | Writes/refreshed cache file | sequential | Creator-scope failures throw; already-open issue is success no-op; cache refresh failure after reopen/comment returns `partial_success` | reopen, integration |
| `issueme_close_issue` | No | REST issue close | Removes cache files | sequential | Creator-scope failures throw; already-closed in-scope issue is success no-op; cache removal failure after close/detection returns `partial_success` | close, integration |
| `issueme_bulk_update_issues` | No | REST issue mutations + optional GraphQL Projects v2 | Refreshes/removes cache for issue actions | sequential | Setup/validation failures throw before run; per-item creator-scope and other failures return aggregate `bulkResults` with `success`, `partial_success`, `failed`, or `skipped` statuses | bulk, integration |
