...
| `issueme_assign_issue` | No | REST assignees | Refreshes cache file | sequential | Throws before remote assignee change; cache refresh failure returns `partial_success` | integration, partial success |
| `issueme_label_issue` | No | REST labels | Refreshes cache file | sequential | Missing-label removal no-op is success; cache refresh failure after change returns `partial_success` | label, integration, partial success |
